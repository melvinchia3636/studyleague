...
import { SERVER_HOST, SERVER_PORT, POCKETBASE_URL } from '../constants/server'
...
export const UserProvider: React.FC<UserProviderProps> = ({ children }) => {
		...
		try {
			...
			if (error.response) {
				...
				switch (error.response.status) {
					...
					case 500:
						throw new Error('Server is having some issues (500). Please try again later.')
					case 404:
	...
	const logout = async () => {
		try {
			...
			if (authToken) {
				try {
					await axios.post(`${SERVER_HOST}:${SERVER_PORT}/api/auth/logout`, {}, {
						headers: { Authorization: `Bearer ${authToken}` }
					})
				} catch (logoutError) {
