...
export const useDiscussions = () => {
	...
	const fetchDiscussions = async () => {
		try {
			...
			setError(err.response?.data?.message || err.message || 'Failed to fetch discussions')
		} finally {
			setLoading(false)
		}
	}

	const createDiscussion = async (discussionData: Omit<Discussion, 'id' | 'created'>) => {
		try {
			...
			throw new Error(err.response?.data?.message || err.message || 'Failed to create discussion')
		}
	}
	...
	const updateDiscussion = async (id: string, updates: Partial<Discussion>) => {
		try {
			...
			throw new Error(err.response?.data?.message || err.message || 'Failed to update discussion')
		}
	}
	...
	const deleteDiscussion = async (id: string) => {
		try {
			...
			throw new Error(err.response?.data?.message || err.message || 'Failed to delete discussion')
		}
	}
...
export const useDiscussionReplies = (discussionId: string | null) => {
	...
	const fetchReplies = async () => {
		...
		try {
			...
			setError(err.response?.data?.message || err.message || 'Failed to fetch discussion replies')
		} finally {
			setLoading(false)
		}
	}
	...
	const addReply = async (replyData: { author: string; body: string }) => {
		...
		try {
			...
			throw new Error(err.response?.data?.message || err.message || 'Failed to add reply')
		}
	}
